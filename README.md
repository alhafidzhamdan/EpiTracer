...
EpiTracer bundles **two independent tools** for whole-genome sequencing (WGS)
data — use either on its own:
...
**An episomal ecDNA caller.**

Flags ecDNA amplicons whose structure fits the *episome* model — a circle
bounded by a duplication (DUP) breakpoint, excised from an otherwise
non-amplified region, often leaving a deletion **"excision scar"**.
...
**A general-purpose, flexible SV + CNV plotter.**

Draws allele-specific copy number and structural variant arcs for one or many
loci on a single axis — with karyotype ideograms, LOH / deletion bars and gene
labels (PDF).
