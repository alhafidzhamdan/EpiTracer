...
generated by simple episomal exclusion**. **For each amplicon called by
[AmpliconArchitect](https://github.com/AmpliconSuite/AmpliconArchitect)** (a
compulsory input), it inspects the structural variant breakpoints at the
amplicon boundaries and flags those whose structure fits the *episome* model of
formation: a circular amplicon bounded by a duplication (DUP) breakpoint,
excised from an otherwise non-amplified chromosomal region, often leaving a
deletion **"excision scar"** at the origin locus.
...
All four inputs are **compulsory**: ecDNA amplicon regions from
