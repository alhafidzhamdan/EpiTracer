## ---------------------------------------------------------------------------
## Montage of the NON-simple episomes: episomal calls whose founder (max-VF)
## junction is NOT a boundary DUP (from validation/nominate_founder_boundary.R) --
## inverted-duplication (INV founder) and DEL/TRA-founder cases. Amplicon zoom,
## low-VF translocations dropped. Title carries the founder class + VF and whether
## an early boundary DUP exists (demoted to cluster 1).
##
## USAGE:
##   Rscript validation/plot_nonsimple_episomes_montage.R \
...
## flag/reject (inverted-duplication, INV founder) + review (bridge, TRA founder);
## DEL founders are accepted as episomes (smaller-circle variants) and excluded here
nom <- fread("validation/output/founder_nomination.tsv")[founder_class %in% c("h2hINV", "t2tINV", "TRA")]
nom[, tier := fifelse(founder_class %in% c("h2hINV","t2tINV"), "1_reject (inverted-dup)", "2_review (TRA bridge)")]
setorder(nom, tier, -founder_vf)
message("flag/reject/review episomes to plot: ", nrow(nom))
...
for (s in unique(nom$WGS_ID)) {
  ...
  for (i in which(nom$WGS_ID == s)) {
    ...
    ttl <- sprintf("%s | %s | %s | founder=%s (VF %.0f) | %s%s",
                   s, ch, ifelse(is.na(gg) | gg == "", "(no oncogene)", gg),
                   nom$founder_class[i], nom$founder_vf[i],
                   ifelse(nom$founder_class[i] %in% c("h2hINV","t2tINV"), "REJECT inverted-dup", "REVIEW TRA bridge"),
                   ifelse(isTRUE(nom$early_boundary_dup[i]), sprintf(", bDUP@cl%s", nom$early_dup_stratum[i]), ""))
    p <- tryCatch(
    ...
    if (!is.null(p)) {
      plots[[length(plots) + 1]] <- p + ggtitle(ttl) + theme(plot.title = element_text(size = 8, face = "bold"))
      message(ttl)
    }
  }
}
...
if (requireNamespace("patchwork", quietly = TRUE) && length(plots)) {
  ...
  outpdf <- "validation/output/flag_reject_review_montage.pdf"
  ...
  for (k in seq(1, length(plots), by = per)) {
    ...
    pg <- patchwork::wrap_plots(grp, ncol = ncol, nrow = nrow) +
      patchwork::plot_annotation(
        title = sprintf("Flagged episomal calls %d-%d of %d (REJECT: inverted-duplication INV founder; REVIEW: TRA-bridge founder)",
